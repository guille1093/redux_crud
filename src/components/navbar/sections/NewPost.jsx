...
function NewPost() {
    ...
    return (
        <>
            <button aria-label="New Post" className="btn btn-ghost btn-circle" onClick={() => modalRef.current.showModal()}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                </svg>
            </button>
            <dialog id="newPost" ref={modalRef} className="modal">
                <div className="modal-box border-base-content border-2">
                    ...
                    <form id="newPost" className="form-control">
                        ...
                        <input accept="image/png, image/jpeg, image/jpg" aria-label="photo" type="file" id="photo" ref={photoRef} name="photo" required className="file-input w-full" onChange={handleImageChange} />
                        {selectedImage && (
                    ...
                    <button className="btn mt-4 btn-outline w-full" onClick={() => modalRef.current.close()}>Cerrar</button>
                </div>
